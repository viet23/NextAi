...
const Dashboard = () => {
  ...
  async function fetchAllPosts(pageId: string, accessToken: string) {
    const fields =
      "id,message,created_time,full_picture,permalink_url,likes.summary(true),comments.summary(true),shares";
    let url = `https://graph.facebook.com/v19.0/${pageId}/posts?fields=${encodeURIComponent(fields)}&limit=100&access_token=${accessToken}`;
  ...
  const fetchFacebookPosts = useCallback(async () => {
    ...
    try {
      ...
      const posts = await fetchAllPosts(accountDetailData.idPage, accountDetailData.accessToken);
